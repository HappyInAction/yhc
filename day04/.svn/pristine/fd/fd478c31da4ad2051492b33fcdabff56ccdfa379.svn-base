...
function butSubmit() {
    var XSAccount = $("#XSAccount").val(),
		XSPassword = $("#XSPassword").val();
    if (XSAccount == "" || XSAccount.indexOf("请输入") > -1 || XSPassword == "" || XSPassword.indexOf("请输入") > -1) {
    ...
    window.location.href = "/InterFace/ValidateCenter/StudentPaper.ashx?type=submit&Account=" + XSAccount +  "&Password=" + XSPassword ;
}

function showImg() {
    $.ajax({
        url: "/InterFace/ValidateCenter/StudentPaper.ashx",
        type: "post",
        dataType: "json",
        cache: false,
        data: {
        ...
        success: function (msg) {
			var imglist = msg.XS;
            $("#XSAccount").val(msg.XSAccount);
			$("#XSPassword").val(msg.XSPassword);
