...
function dealHasReport(msg) {
    //错误弹窗
    if (errShow(msg) || usermsgflag == 1) {
        return false;
    }

    if (msg.result == "true") {
        jxl.collectedFlag = 1;//已采集数据
        ...
        jxl.getUserMsgFn(dealGetUserMsg);

        jxl.getDataSourceFn(dealGetDataSource);
    }
}


function dealGetDataSource(msg) {
    ...
    {
        uploadImgShow("系统繁忙，请稍后重试", "000000");
        return false;
    }
...
function dealToken(msg) {
    ...
    if (status != 1)//运营商不支持或已下线
    {
        ...
        {
            window.location.href = "http://tz121776564.oicp.net/template/ValidateBusiness.html?token=" + jxl.defaultReqMsg.token + "&website=" + jxl.defaultReqMsg.website;
        }
    }
}
...
function dealReq(msg) {
    ...
    {
        uploadImgShow("密码错误，请重新输入", data.process_code);
        $("#phone").val("");
    }
    ...
    {
        uploadImgShow("验证失败，请修改密码后重试", data.process_code);
    }
    else if (data.process_code == 30000 || data.process_code == 0)//采集请求超时
    {
        uploadImgShow("验证失败，请稍后再试", data.process_code);
    }
...
function dealSetFlag(msg) {
    ...
    window.location.href = "http://tz121776564.oicp.net/InterFace/ValidateBusiness.ashx";
...
function dealResetReq(msg) {
    ...
    {
        uploadImgShow("建议按下图方式找回服务密码", data.process_code);
    }
    ...
    {
        uploadImgShow("重置密码失败，请稍后再试", data.process_code);
    }
...
function msg_req(type_in) {
    if (usermsgflag == 1)//返回错误信息
    {
        uploadImgShow("系统繁忙，请稍后重试", "000001");
        return false;
    }
...
function dealSetFlag_bus(msg) {
    ...
    window.location.href = "http://tz121776564.oicp.net/InterFace/ValidateEmail.ashx";
}
...
function dealCollectReq_bus(msg) {
    ...
    if (datasource.success != true) //请求失败
    {
        turnImgShow("系统繁忙，请稍后重试", "000000");
        return false;
    }
    var data = datasource.data;
    if (data.process_code == 10003)//密码错误
    {
        turnImgShow("密码错误，请重新输入", data.process_code);
        $("#buspw").val("");
    }
    else if (data.process_code == 10004 || data.process_code == 10006)//短信验证码错误/失效
    {
    ...
    {
        turnImgShow("验证失败，请修改密码后重试", data.process_code);
    }
    else if (data.process_code == 30000 || data.process_code == 0)//超时
    {
        turnImgShow("验证失败，请稍后再试", data.process_code);
    }
    else if (data.process_code == 10002 || data.process_code == 10001)//输入短信验证码
    {
        jxl.captchaFlag = 1;
