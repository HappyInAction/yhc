...
function butSubmit() {
    var score = $("#score").val();
    if (score == "" || score.indexOf("请输入") > -1 || !chk_number(score)) {
        errShow({ "errmsg": "请输入芝麻信用分", "errcode": "" });
        return;
    ...
    window.location.href = "/InterFace/ValidateSesame.ashx?type=submit&score=" + score;
}


function showScore() {
    $.ajax({
        url: "/InterFace/ValidateSesame.ashx",
        type: "post",
        dataType: "json",
        cache: false,
        data: {
