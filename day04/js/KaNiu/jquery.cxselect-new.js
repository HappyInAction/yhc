	...
	$.cxSelect = function(settings){
		...
		cxSelect.init = function(){
			...
			_this.settings = $.extend({}, $.cxSelect.defaults, settings, {
				...
				IsDisable: _this.dom.box.data('IsDisable')
			});
		...
		cxSelect.buildContent = function(){
			var _this = this;
			var _IsDisable = this.settings.IsDisable;
			...
			if (_IsDisable == true) {
			    _this.selectArray[0].html(_html).prop('disabled', true).trigger('change');
			}
			else {
		...
		cxSelect.selectChange = function(name){
		    ...
		    // 遍历数据写入下拉选框
			var IsDisable = this.settings.IsDisable;
			if (this.selectArray[selectNext]) {
			    _html = this.getNewOptions(this.selectArray[selectNext], selectData);
			    if (IsDisable == true) {
			        this.selectArray[selectNext].html(_html).prop('disabled', true).css({ 'display': '', 'visibility': '' }).trigger('change');
			    }
			    else {
